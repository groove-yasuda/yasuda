        ...
        methods: {
                ...
                axios.request({
                    ...
                    .then((response) => {
                        if (response.data === 0) {
                            ...
                            this.errorMessage = '社員IDが登録されています';
                            this.idError = false;
                        }
                    })
            },
            ...
            UPDATE() {
                axios
                    ...
                    .then((response) => {
                        if (response.data === 0) {
                            this.$router.push({ name: 'upd_Confi', params: { empId_Before: this.empId, empName_Before: this.empName, empId_After: this.inputEmpId, empName_After: this.inputEmpName } });
                        }
                        else if (response.data === 1) {
                            this.errorMessage = '社員IDと社員名の組み合わせが登録されています';
